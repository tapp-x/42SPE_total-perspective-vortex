...
def extract_features_from_edf(filepath, tmin=-0.5, tmax=4.0, plot=False):
    ...
    target_events = {}
    if 'T1' in event_dict: target_events['T1'] = event_dict['T1']
    if 'T2' in event_dict: target_events['T2'] = event_dict['T2']
...
def preprocessing(subject_id, runs, base_path=None, plot=False):
    ...
    print(f"\n=== Loading subject {subject_folder} | Targeted runs : {runs} ===")
    ...
    for run in runs:
        ...
        if X_run is not None and y_run is not None:
            X_list.append(X_run)
            y_list.append(y_run)
            print(f"[+] Run {run:02d} loaded : {X_run.shape[0]} events.")
